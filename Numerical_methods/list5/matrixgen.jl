     ...
     error("size n should be > 1")
        ...
        for k in 1:nb
            ...
            for i in 1:l, j in 1:l
                ...
                push!(indexes_pairs, [(k-1)*l+i, (k-1)*l+j])
                append!(values, Ak[i,j])
            end
            if k<nb
                for i in 1:l
                    # println(f,(k-1)*l+i," ",k*l+i," ",0.3*rand())
                    push!(indexes_pairs, [(k-1)*l+i, k*l+i])
                    append!(values, 0.3*rand())
                    end
            end
            if k>1
            for i in 1:l
                #  println(f,(k-1)*l+i," ",(k-1)*l," ",0.3*rand())
                    push!(indexes_pairs, [(k-1)*l+i, (k-1)*l])
                    append!(values, 0.3*rand())
                end
            end 
        end
		# end	 # do
        return indexes_pairs, values
	end # blockmat
